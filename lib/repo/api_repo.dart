...
class ApiRepo {
  ...
  Future<List<TodosModel>?> getchatsWithModel(String number) async {
    try {
      log("our value is $number");
      List<Map<String, dynamic>>? data;

      var result = await URL.get(Uri.parse("${AppUrls.getApi}"));
      ...
      if (result.statusCode == 200 || result.statusCode == 201) {
        ...
        log(data.toString());
        return data.map((e) => TodosModel.fromMap(e)).toList();
      } else {
        log("error");
      }
      return null;
